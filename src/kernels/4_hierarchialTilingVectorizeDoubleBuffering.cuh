...
__global__ void __launch_bounds__(NUM_THREADS)
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    ...
    for (uint warpKidx = 0; warpKidx < BK; warpKidx += WK) {
      ...
      for (uint wSubRowIdx = 0; wSubRowIdx < WMITER; ++wSubRowIdx) {
        wmma::load_matrix_sync(a_frag[wSubRowIdx], &As[(bkIdx/BK) % 2][warpRow * WM * BK + warpKidx + wSubRowIdx * mma_m * BK], BK);
        // AsWarpLocalInBlock += mma_m * BK;
      }
      for (uint wSubColIdx = 0; wSubColIdx < WNITER; ++wSubColIdx) {
        wmma::load_matrix_sync(b_frag[wSubColIdx], &Bs[(bkIdx/BK) % 2][warpCol * WN + warpKidx * BN + wSubColIdx * mma_n], BN);
