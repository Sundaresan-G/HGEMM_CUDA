...
__global__ void __launch_bounds__(NUM_THREADS)
    hgemmHierarchialTilingVectorizeTransposed(int M, int N, int K, half alpha, half *A, half *B,
                    half beta, half *C) {
  ...
  wmma::fragment<wmma::matrix_a, mma_m, mma_n, mma_k, half, wmma::col_major> a_frag[WMITER];
  ...
  for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
    ...
    for (uint offset = 0; offset + rowStrideA <= BM; offset += rowStrideA) {
      ...
      half temp[8]; 
      reinterpret_cast<float4 *>(temp)[0] = reinterpret_cast<float4 *>(&A[(innerRowA + offset) * K + innerColA * 8])[0];

      // Store them in SMEM in transposed form
      for (int index = 0; index < 8; index++) {
        As[(innerColA * 8 + index) * BM + (innerRowA + offset)] = temp[index];
      }
    }

    for (uint offset = 0; offset + rowStrideB <= BK; offset += rowStrideB) {

      reinterpret_cast<float4 *>(&Bs[(innerRowB + offset) * BN + innerColB * 8])[0] 
